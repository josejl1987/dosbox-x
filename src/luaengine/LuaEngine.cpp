...
void LuaEngine::LuaInstructionHook() {
    // LRDB poll — thin, kept here for VS Code debug adapter responsiveness
    if (lrdb_server_ && lrdb_active_) {
        lrdb_server_->command_stream().poll();
    }
    // All breakpoint/trace/run-until/CDL logic now dispatched via
