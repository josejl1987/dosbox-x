...
RamSearchWindow::RamSearchWindow(LuaEngineDebugTools::DebuggerSession* session) 
    : engine_(session ? session->ramSearch() : nullptr),
...
void RamSearchWindow::initialize(LuaEngineMemoryDomains::MemoryDomainManager* memory_mgr) {
    // ponytail: engine_ is now owned by DebuggerSession; this method is a no-op
    // for backward compat. Session calls engine_->initialize() directly.
    (void)memory_mgr;
}

void RamSearchWindow::render() {
    if (!show_window_) return;
    if (!engine_) return; // null-guard: session not initialized
