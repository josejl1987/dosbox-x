...
static void ImGui_ImplSDL2_CreateWindow(ImGuiViewport* viewport)
{
    ...
    if (use_opengl)
    {
        ...
        // PERFORMANCE OPTIMIZATION: Disable VSync for secondary viewports
        // This significantly improves multiviewport performance
...
static void ImGui_ImplSDL2_SwapBuffers(ImGuiViewport* viewport, void*)
{
    ...
    if (vd->GLContext)
    {
        ...
        // PERFORMANCE OPTIMIZATION: Ensure VSync is disabled for this viewport
        // Check swap interval and disable if not already done
        static bool vsync_warning_shown = false;
        if (SDL_GL_GetSwapInterval() != 0) {
            if (SDL_GL_SetSwapInterval(0) != 0 && !vsync_warning_shown) {
                // Log warning only once to avoid spam
                fprintf(stderr, "Warning: Could not disable VSync for secondary viewport\n");
                vsync_warning_shown = true;
            }
        }
